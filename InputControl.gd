...
func _ready() -> void:
	...
	self.icon.frame = self._get_current_icon_index(self.device_id, self.action_name)
...
func _joy_connection_changed(device_id : int, connected : bool) -> void:
	if connected:
		...
		self.device_id = -1
	self.icon.frame = self._get_current_icon_index(self.device_id, action_name)	


	
func _get_current_icon_index(device_id : int, action_name : String) -> int:
